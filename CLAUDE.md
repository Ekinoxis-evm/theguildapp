...
- **UI system** (see `.claude/skills/ui-system` + `docs/UX_BACKLOG.md`): actions use `.btn .btn-primary|-outline|-ghost` (never underlined text for actions); consoles use `<Tabs>` (`src/components/tabs.tsx`, URL-synced); style vocabulary via `STYLE_TAGS` + `<StyleChips>`; ratings via `src/components/rating.tsx`. **The bottom nav owns navigation** — no "← Dashboard" links; contextual backs are `btn-ghost`; the home shows only cards the role's nav doesn't already cover; sign-out lives on /profile.
- **Migrations without a Supabase token**: the management token is not kept on disk. Apply via the dashboard SQL editor in the Chrome tab (paste migration SQL + insert into `supabase_migrations.schema_migrations` + `notify pgrst, 'reload schema'`), and hand-write the `database.types.ts` entry (regen when a token exists must match). Dashboard config (email templates!) does NOT survive project pause/restore — recheck after any restore.
