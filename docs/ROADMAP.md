...
| 7.3 | Reviews & ratings: stars + comment after completed paid bookings only | done (2026-08-03) — verified-purchase trigger, rate/edit on /bookings history, ★ badge + review list on shop page, barber profile, and shops list — live in prod 2026-08-04 (migration applied post-restore via dashboard SQL editor; E2E verified: ★4.5 showing on test shop) |
...
| # | Item | Status |
|---|---|---|
| 8.1 | Barber social hub: profile links (Instagram, YouTube, TikTok, external booking incl. Squire) — "one link in the bio"; Guild profile as the barber's professional identity, native booking optional | done (2026-08-04) — `barber_links` + editor in /my-barber + LinkHub buttons on public profile; app-wide `.btn` system (primary/outline/ghost, press feedback) replaced text-like actions |
...
| 5.9 | Ambient split: dedicated demo environment (own Supabase project `theguildapp-demo`, Stripe test-mode forever, `demo` branch with Preview-scoped Vercel env vars) | **deferred — pre-launch step** (founder 2026-07-21: one project is fine while there are no real users). Groundwork done: architecture in SETUP.md, `pnpm seed:demo` env-parameterized. To execute: settle Supabase org invoice (creation is blocked by it) → create project → push migrations → seed → scope Preview env vars → cut `demo` branch |
