...
| `SUPABASE_SECRET_KEY` | same (secret key) — **server only** | secret |
...
| `STRIPE_SECRET_KEY` / `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` | Stripe dashboard (test mode until Phase 3) | Phase 3 |
| `STRIPE_WEBHOOK_SECRET` | `stripe listen --forward-to localhost:3000/api/webhooks/stripe` | Phase 3 |
