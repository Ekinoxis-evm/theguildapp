...
export default async function MyBarberPage() {
  ...
  if (!barber) {
    return (
      <main className="mx-auto w-full max-w-md flex-1 px-6 py-16">
        <BackLink />
        <h1 className="mt-2 text-2xl font-semibold">Become a Guild private barber</h1>
        <p className="mt-2 text-sm text-neutral-600">
          Serve premium clients at their homes. The Guild team reviews every
          application.
        </p>
  ...
  const [{ data: coverage }, { data: services }, { data: bookings }] =
    await Promise.all([
      supabase
        .from("coverage_areas")
        .select("*")
  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-16">
      <BackLink />
      <h1 className="mt-2 text-2xl font-semibold">Private barber profile</h1>
