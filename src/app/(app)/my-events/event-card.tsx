    ...
    <section className="border border-neutral-800 p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs uppercase tracking-wide text-guild-yellow">{status}</p>
        </div>
        {next && (
          <button
            disabled={busy}
      ...
      {status === "live" && (
        <div className="mt-4 flex items-center gap-4 border border-neutral-800 bg-neutral-800 p-3">
          {/* eslint-disable-next-line @next/next/no-img-element -- data URL */}
          <img src={event.qrDataUrl} alt="Registration QR code" className="h-28 w-28" />
          <div className="text-xs text-neutral-400">
            <p className="font-medium text-white">Registration QR</p>
      ...
      <ul className="mt-2 space-y-1">
        {attendees.map((a) => (
          <li
            key={a.profile_id}
            className="flex items-center justify-between gap-3 border border-neutral-800 px-3 py-2 text-sm"
          >
            <span>
            ...
            {!a.service_claimed_at && status === "live" && (
              <button
                ...
                className="shrink-0 border border-neutral-800 px-2 py-1 text-xs"
              >
                Mark served
              </button>
            )}
